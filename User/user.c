...
//    signal(SIGALRM, alarmHandler);
    ...
    int amount_to_claim = 0, amount_claimed = 0, resource_to_claim = 0;
...
//                    perror("user sem_wait");
                ...
                if (RCB_array[resource_to_claim].currentResourceCount < amount_to_claim) {
                    ...
                    RCB_array[resource_to_claim].currentResourceCount -= RCB_array[resource_to_claim].currentResourceCount;
...
/* Delete */
void deleteFromQueue(rcb_t *block, int resource) {
    pid_t pid = getpid();
